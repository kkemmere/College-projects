...
public class collectnodes {
	...
	{
		Scanner inp = new Scanner(new File("Edge.dat"));
	...
	/*	
		...
		System.out.println();
		*/
		System.out.println("Final Exam Part");
		System.out.println();
		...
		for (int j = 0; j < count; j++)
		{
			if (nodes[j].average > temp)
			{
				tempedges = nodes[j].connect;
				tempnoded = nodes[j].name;
			}
				
		}
		
		System.out.println("Node with highest degree is " + tempnoded + " which has " + tempedges + " edges");
	...
	//	System.out.println(nodee);
		...
		System.out.println("Removing node " + tempnoded + " leaves nodes " + tempnodef + " and " + nodee + " without any edges connecting to it");
